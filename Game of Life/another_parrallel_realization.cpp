...
void setup_boundaries_mpi(int n, int * data, int rank, int p) {
    ...
    MPI_Sendrecv(&data[1], 1, column, left, 0, &data[n + 1], 1, column, right, 0, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&data[n], 1, column, right, 0, &data[0], 1, column, left, 0, MPI_COMM_WORLD, &status);
...
void collectdata(int *data, int n, int p, int rank) {
    ...
    if (rank == 0) {
        ...
        for(int i = 0 ; i < p; ++i) {
            for(int j = 0 ; j < p; ++j) {
                MPI_Recv(&(data[(i * p + j) / p * (N + 2) * n + (i * p + j) % p * n  + N + 2 + 1]), 1, blockrecv, i * p + j + 1, 0, MPI_COMM_WORLD, &status);
            }
        }
    } else {
...
void run_life(int n, int T, int rank, int size)
{
    ...
    if (rank == 0) {
        ...
        ofstream stats("stat1.txt", std::ofstream::out | std::ofstream::app);
